import { EventsController, animationFrameThrottle, computed, effect, getScope, isDOMNode, isFunction, isKeyboardClick, isTouchEvent, listenEvent, onDispose, scoped, setAttribute, setStyle, signal, toggleClass } from "./vidstack-BwClSYSC.js";
...
function cloneTemplate(template, length, onCreate) {
	...
	if (content?.nodeType !== 1) throw Error("[vidstack] template must contain root element");
...
function useActive($el) {
	...
	return computed(() => {
		const isMouseEnter = $isMouseEnter(), isFocusIn = $isFocusedIn();
