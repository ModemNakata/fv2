import { Component, DOMEvent, EventsController, EventsTarget, State, ViewController, animationFrameThrottle, camelToKebabCase, computed, deferredPromise, effect, functionDebounce, functionThrottle, isArray, isKeyboardClick, isKeyboardEvent, isNumber, isString, isUndefined, listenEvent, method, noop, onDispose, peek, prop, provideContext, scoped, setAttribute, setStyle, signal, tick, untrack, uppercaseFirstChar, waitIdlePeriod } from "./vidstack-BwClSYSC.js";
import { ListSymbol } from "./vidstack-BzoKHzuo.js";
import { IS_CHROME, IS_IOS, IS_IPHONE, canChangeVolume, canGoogleCastSrc, canOrientScreen, canPlayAudioType, canPlayVideoType, isAudioSrc, isDASHSrc, isDASHSupported, isHLSSrc, isHLSSupported, isVideoSrc } from "./vidstack-gDipnrck.js";
import { TimeRange, getTimeRangesEnd, getTimeRangesStart, updateTimeIntervals } from "./vidstack-DhHK6ZQ-.js";
import { getRequestCredentials, preconnect } from "./vidstack-CgNu5UhN.js";
import { TextTrack, TextTrackSymbol, isTrackCaptionKind } from "./vidstack-BpQc-xfw.js";
import { mediaContext, useMediaContext } from "./vidstack-B6lBWEKu.js";
import { clampNumber, round } from "./vidstack-CZQo98ic.js";
import { isHTMLElement, isTouchPinchEvent, prefersReducedMotion, setAttributeIfEmpty } from "./vidstack-CToJFZVs.js";
import { QualitySymbol } from "./vidstack-BkDwYdN3.js";
import { coerceToError } from "./vidstack-CDOoBnJL.js";
...
//#endregion
//#region src/core/api/player-state.ts
...
var VimeoProviderLoader = class {
	...
	async loadPoster(src, ctx, abort) {
		const { resolveVimeoVideoId, getVimeoVideoInfo } = await import("./vidstack-q-9soyHT.js").then((n) => n.utils_exports);
...
var YouTubeProviderLoader = class {
	...
	async loadPoster(src, ctx, abort) {
		const { findYouTubePoster, resolveYouTubeVideoId } = await import("./vidstack-IMa-zkAV.js").then((n) => n.utils_exports);
...
//#endregion
//#region src/core/state/media-events-logger.ts
const MEDIA_EVENTS = [
	"abort",
	"can-play",
	"can-play-through",
	...
	async requestGoogleCast(trigger) {
		try {
			...
			if (!this.#googleCastLoader) {
				const $module = await import("./vidstack-AIqRqNdI2.js");
...
//#endregion
//#region src/core/state/tracked-media-events.ts
...
var MediaStateManager = class extends MediaPlayerController {
	...
	["media-type-change"](event) {
		...
		if (viewType !== currentViewType) setTimeout(() => {
			requestAnimationFrame(() => {
				if (!this.scope) return;
...
//#endregion
//#region src/foundation/logger/colors.ts
...
//#endregion
//#region src/foundation/logger/log-level.ts
...
//#endregion
//#region src/foundation/logger/ms.ts
const s = 1e3;
const m = s * 60;
const h = m * 60;
const d = h * 24;
...
//#endregion
//#region src/foundation/logger/log-printer.ts
