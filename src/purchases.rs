...
use crate::entity::{content_items, images, user_purchases, users};
use crate::gallery;

// ---- Page template ----

#[derive(Template)]
#[template(path = "purchased.html")]
struct PurchasedPage {
    username: String,
    logged_in: bool,
...
#[derive(Template)]
#[template(path = "components/purch-video-card.html")]
struct PurchVideoCardTemplate {
    v: PurchVideoItem,
}

#[derive(Template)]
#[template(path = "components/purch-gallery-card.html")]
struct PurchGalleryCardTemplate {
    g: PurchGalleryItem,
}
...
pub async fn purchased(
    ...
    let all = UserPurchases::find()
        .filter(user_purchases::Column::UserId.eq(user_id))
        .all(&state.conn)
        .await
        .map_err(actix_web::error::ErrorInternalServerError)?;

    let content_ids: Vec<Uuid> = all.iter().map(|f| f.content_id).collect();
    ...
    .render()
    .expect("purchased.html should be valid");
...
pub async fn api_purchased(
    ...
    let purchases = UserPurchases::find()
        .filter(user_purchases::Column::UserId.eq(user_id))
        .order_by_desc(user_purchases::Column::CreatedAt)
        .all(&state.conn)
        .await;
    ...
    let purchases = match purchases {
        Ok(p) => p,
        Err(e) => {
            log::error!("DB error querying purchases: {e}");
    ...
    let all_content_ids: Vec<Uuid> = purchases.iter().map(|p| p.content_id).collect();
    ...
    let type_ids: Vec<Uuid> = purchases
        .iter()
        .filter_map(|p| {
            content_map
                .get(&p.content_id)
