...
`MediaAttachMixin` creates a class that registers itself with the [provider](./provider-mixin.md) on connect. Apply it to custom media elements so they automatically wire to the store.
