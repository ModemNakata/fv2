...
*   **`playerContext`** — the store, consumed by [`PlayerController`](./player-controller.md) and other controllers
...
Use `ProviderMixin` separate from [`ContainerMixin`](./container-mixin.md) when the store owner is a different element from the container:
...
`ProviderMixinConfig<PlayerStore>`
...
Provider configuration with contexts and store factory.
